 ...
 * 
/*#define NO_CRYPT /*#define NPDRM /*#define SPRX
 */
...
u8   sdkversion_static[] = {
	...
	/*
	 * 0x00,0x00,0x00,0x01,0x00,0x00,0x00,0x01,0x00,0x00,0x00,0x30,0x00,0x
	 * 00,0x00,0x00
	 */
};
...
enumerate_segments()
{
	...
	for (i = 0; i < get_u16(&(input_elf_header->e_phnum)); i++) {
		...
		/*
		 * segment_ptr->compressed = (i<2); segment_ptr->incrypt =
		 * (i<6); /* **TESTING segment_ptr->encrypted = (i<5);
		 */
		...
		/*
		 * if(i==0) { segment_ptr->padding = 0x26A4; } else if(i==1)
		 * { segment_ptr->padding = 0xC; } else {
		...
		/*
		 * if(segment_ptr->len == 0x14BCC8) { segment_ptr->padding +=
		 * 0x4330; }
		 */

		printf("processing segment %d with rlen %x len %x offset %x...", i, segment_ptr->rlen, segment_ptr->len, in_data_offset);
		fflush(stdout);
...
void 
init_Self_NPDRM(Self_NPDRM * npdrm, char *titleid, char *filename)
{
...
build_segment_crypt_data()
{
	...
	while (segment_ptr != NULL) {
		if (segment_ptr->incrypt) {
			...
			memcpy(&segment_crypt_data[segment_crypt_data_len], segment_ptr->crypt_segment.sha1, sizeof(segment_ptr->crypt_segment.sha1));
			segment_crypt_data_len += sizeof(segment_ptr->crypt_segment.sha1);
			memcpy(&segment_crypt_data[segment_crypt_data_len], segment_ptr->crypt_segment.hmac, sizeof(segment_ptr->crypt_segment.hmac));
			segment_crypt_data_len += sizeof(segment_ptr->crypt_segment.hmac);

			if (segment_ptr->encrypted) {
				set_u32(&(segment_ptr->enc_segment.segment_erk_index), segment_crypt_data_len / 0x10);
				memcpy(&segment_crypt_data[segment_crypt_data_len], segment_ptr->crypt_segment.erk, sizeof(segment_ptr->crypt_segment.erk));
				segment_crypt_data_len += sizeof(segment_ptr->crypt_segment.erk);
				set_u32(&(segment_ptr->enc_segment.segment_riv_index), segment_crypt_data_len / 0x10);
				memcpy(&segment_crypt_data[segment_crypt_data_len], segment_ptr->crypt_segment.riv, sizeof(segment_ptr->crypt_segment.riv));
				segment_crypt_data_len += sizeof(segment_ptr->crypt_segment.riv);
			} else {
...
main(int argc, char *argv[])
{
	...
	/* create self headers */
	Self_Shdr       output_self_header;
	memset(&output_self_header, 0, sizeof(output_self_header));
	Self_Ehdr       output_extended_self_header;
	memset(&output_extended_self_header, 0, sizeof(output_extended_self_header));
	Self_Ihdr       output_self_info_header;
	memset(&output_self_info_header, 0, sizeof(output_self_info_header));
	...
	segment_certification_header segment_header;
	...
	/* generate keys and do signing stuff */
	metadata_crypt_header md_header;
	memset(&md_header, 0, sizeof(md_header));
